...
const darkModeInput = "block w-full bg-transparent px-2 py-1 resize-none outline-none text-white placeholder:text-gray-400 border-t-0 border-x-0 border-b-2 border-gray-300 focus:border-[#D06D3D] focus:ring-0"

function Input({ className, type, variant, ...props }: React.ComponentProps<"input"> & { variant?: "login" | "dashboard" | "ai" | "note" | "darkMode" }) {
  return (
    <InputPrimitive
      type={type}
      data-slot="input"
      className={cn(
        ...
        variant == "darkMode" && darkModeInput,
